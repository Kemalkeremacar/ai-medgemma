...
class DemoHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        parsed = urlparse(self.path)
        path = parsed.path
        if path != "/api/oneri-ai/chat":
            self._send_json({"error": "not_found"}, 404)
            return
        length = int(self.headers.get("Content-Length") or 0)
        raw = self.rfile.read(max(0, min(length, 200_000))) if length else b"{}"
        try:
        ...
        store = self.store
        sources = ["Snapshot özeti"]
        reply_parts = [
            "Yerel demo sunucusu: MedGemma / provizyon köprüsü yok; elinizdeki handoff özeti:",
            "",
        ]
        if proposal_id:
            ...
            if detail:
                ...
                sources = ["Kural önerileri"]
                if detail.get("officialEvidence"):
                    sources.append("Resmî evidence")
                reply_parts.append(
                    f"• `{proposal_id}` — {proc.get('listeTipi') or '?'} "
                    f"{proc.get('kod') or ''} {proc.get('ad') or ''} "
                    f"| tip: {p.get('targetRuleType')} | öncelik: {p.get('priority')}"
                )
            else:
                reply_parts.append(f"• Proposal bulunamadı: {proposal_id}")
        else:
            listing = store.list_proposals(q=message[:40], page=1, page_size=3)
            sources = ["Kural önerileri", "HUV/SUT listeleri"]
            reply_parts.append(f"• Arama sonucu: {listing.get('total', 0)} aday")
            for item in listing.get("items") or []:
                reply_parts.append(
                    f"  - {item.get('proposalId')}: {item.get('listeTipi')} "
                    f"{item.get('procedureKod')} — {item.get('procedureAd')}"
                )
        reply_parts.append("")
        reply_parts.append(
            "Tam Öneri AI (MedGemma + provizyon) için Provizyon paneli üzerinden kullanın."
        )
        self._send_json(
            {
                "reply": "\n".join(reply_parts),
                ...
                "disclaimer": (
                    ...
                    "HUV ve SUT ayrı değerlendirilir."
                ),
            }
        )
